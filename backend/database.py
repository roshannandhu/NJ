...
# ── lightweight migrations (no Alembic — this app ships stdlib-only) ──────────
# New columns added to existing tables won't appear via create_all() on a DB
# that predates them, so add them here. SQLite's ADD COLUMN is cheap and the
# whole thing is idempotent: we only add columns that are actually missing.
_EXPECTED_COLUMNS = {
    "quotations": [
        ("updated_at", "DATETIME"),
...
def ensure_columns():
    """Add any missing columns to existing tables. Safe to call on every start."""
